    ...
    def graph_call(self, repo, org):
        ...
        try:
            if git_content['data']['organization']['repository']['description']:
                if "_" in repo: repo = repo.replace("_", "-")
            ...
            if "-" in repo:
                repo = repo.replace("-", "_")
                self.graph_call(repo, org)  # assuming, in recursion if ['description'] NA then ['re..Topics'] NA
                return
            if "_" in repo: repo = repo.replace("_", "-")
